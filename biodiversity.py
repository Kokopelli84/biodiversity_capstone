...
category_pivot['percent_protected'] = category_pivot.apply(lambda row: row.protected / float(row.protected + row.not_protected), axis = 1)
...
#contingency = category_pivot[['protected', 'not_protected']].values
#print contingency
contingency = [[79,442],[38,176]]
...
min_detectable_effect = 100 * (0.1 - 0.15) / 0.15 * -1
...
import math
obs_req_bryce = math.ceil(sample_size / float(250))
obs_req_yellostown = math.ceil(sample_size / float(507))
print str(obs_req_bryce) + ' weeks of sampling required for Bryce National Park'
print str(obs_req_yellostown) + ' weeks of sampling required for Yellowstone National Park'
